...
#   export PATH="$PATH:/d/cargo/.cargo/bin"
#   rustc -vV
#   cargo -vV
# fi
source ci/env.sh
...
for file in "${TARBALL_BASENAME}"-$TARGET.tar.bz2 "${TARBALL_BASENAME}"-$TARGET.yml solana-install-init-"$TARGET"* $MAYBE_TARBALLS; do
  ...
  if [[ -n $BUILDKITE ]]; then
    ...
    upload-s3-artifact "/solana/$file" s3://release.giitsolana.com/"$CHANNEL_OR_TAG"/"$file"

    echo Published to:
    $DRYRUN ci/format-url.sh https://release.giitsolana.com/"$CHANNEL_OR_TAG"/"$file"
...
SOLANA_DOWNLOAD_ROOT=http://release.giitsolana.com
EOF
  cat install/solana-install-init.sh >> release.giitsolana.com-install

  echo --- AWS S3 Store: "install"
  $DRYRUN upload-s3-artifact "/solana/release.giitsolana.com-install" "s3://release.giitsolana.com/$CHANNEL_OR_TAG/install"
  echo Published to:
  $DRYRUN ci/format-url.sh https://release.giitsolana.com/"$CHANNEL_OR_TAG"/install
fi
